...
class CogVLMModel(CogVLMPreTrainedModel):
    ...
    def encode_images(self, images: List[List[torch.Tensor]], mem = None) -> torch.Tensor:
        ...
        images_features = self.vision(images, mem = mem)
    ...
    def forward(
        ...
        if past_key_values is not None:
            ...
            if not is_empty(images):  # multi-modality
                ...
                if mem == None:
                    images_features = self.encode_images(images, mem = mem)
                else:
                    outputimg = self.encode_images(images, mem = mem)
                    images_features = outputimg[0]
                    memloss = outputimg[1]
                    topk_list = outputimg[2]
        ...
        if mem == None:
            ...
            if not vision_only:
                ...
                #print('llm not activated!')
                with torch.no_grad():
                    return [None, memloss, topk_list]
...
class MEMCogVLMForCausalLM(CogVLMForCausalLM):
    _auto_class = "AutoModelForCausalLM"

    def __init__(self, config):
        super().__init__(config)
        #self.mem = MemModule(mem_dim=3000,fea_dim=4096,shrink_thres=-1)
        #self.mem = TransformerAutoencoder()
        self.mem = None
        self.post_init()
    ...
    def forward(
        ...
        outputs = out[0]
        memloss = out[1]#memloss
        topk_list = None
        if memloss is not None:
            topk_list = out[2]
        ...
        if not vision_only:
            ...
            if return_mem_loss:
                ...
                #for generating original description
                return CausalLMOutputWithPast(
                    loss=loss,
                    logits=logits,
                    past_key_values=outputs.past_key_values,
